...
MODEL_TYPES = ("Default",)
MODEL_COLORS = ("Gray", "Black", "White", "Purple", "Green", "Red", "Blue")
DEFAULT_MODEL_TYPE = "Default"
DEFAULT_MODEL_COLOR = "Blue"
MODEL_TYPE_IDS = {name: index for index, name in enumerate(MODEL_TYPES)}
MODEL_COLOR_IDS = {name: index for index, name in enumerate(MODEL_COLORS)}
...
FRMT_AVATAR_HEADER_LEGACY = "!4siHHI"
FRMT_AVATAR_HEADER = "!4siHHIBB"
AVATAR_HEADER_PACKET_SIZE = struct.calcsize(FRMT_AVATAR_HEADER)
AVATAR_HEADER_LEGACY_PACKET_SIZE = struct.calcsize(FRMT_AVATAR_HEADER_LEGACY)
AvatarHeaderPacket = Tuple[bytes, int, int, int, int, str, str]
...
# Legacy GEND row: player_id, placement, name only.
FRMT_GEND_ITEM_LEGACY = "!iB32s"
# v2 row: player_id, placement (255 = none), platforms reached, elapsed centiseconds from match start, name.
FRMT_GEND_ITEM = "!iBHI32s"
GEND_ITEM_LEGACY_SIZE = struct.calcsize(FRMT_GEND_ITEM_LEGACY)
GEND_ITEM_SIZE = struct.calcsize(FRMT_GEND_ITEM)
FRMT_KICK = "!4sii"
FRMT_KICKED = "!4sB"
...
def normalize_model_type(model_type: str) -> str:
    for value in MODEL_TYPES:
        if value.casefold() == (model_type or "").casefold():
            return value
    return DEFAULT_MODEL_TYPE


def normalize_model_color(model_color: str) -> str:
    for value in MODEL_COLORS:
        if value.casefold() == (model_color or "").casefold():
            return value
    return DEFAULT_MODEL_COLOR


def model_type_id(model_type: str) -> int:
    return MODEL_TYPE_IDS.get(normalize_model_type(model_type), MODEL_TYPE_IDS[DEFAULT_MODEL_TYPE])


def model_color_id(model_color: str) -> int:
    return MODEL_COLOR_IDS.get(normalize_model_color(model_color), MODEL_COLOR_IDS[DEFAULT_MODEL_COLOR])


def model_type_name(type_id: int) -> str:
    if 0 <= int(type_id) < len(MODEL_TYPES):
        return MODEL_TYPES[int(type_id)]
    return DEFAULT_MODEL_TYPE


def model_color_name(color_id: int) -> str:
    if 0 <= int(color_id) < len(MODEL_COLORS):
        return MODEL_COLORS[int(color_id)]
    return DEFAULT_MODEL_COLOR
...
def pack_avatar_header(
    player_id: int,
    avatar_id: int,
    total_chunks: int,
    payload_size: int,
    model_type: str = DEFAULT_MODEL_TYPE,
    model_color: str = DEFAULT_MODEL_COLOR,
) -> bytes:
    ...
    return struct.pack(
        ...
        model_color_id(model_color),
    )


def safe_unpack_avatar_header(data: bytes) -> Optional[AvatarHeaderPacket]:
    if len(data) == AVATAR_HEADER_LEGACY_PACKET_SIZE:
        try:
            tag, player_id, avatar_id, total_chunks, payload_size = struct.unpack(FRMT_AVATAR_HEADER_LEGACY, data)
        except struct.error:
            return None
        ...
        if total_chunks > AVATAR_MAX_CHUNKS or payload_size > NETWORK_AVATAR_BYTES:
            return None
        return tag, player_id, avatar_id, total_chunks, payload_size, DEFAULT_MODEL_TYPE, DEFAULT_MODEL_COLOR
    if len(data) != AVATAR_HEADER_PACKET_SIZE:
        return None
    try:
        tag, player_id, avatar_id, total_chunks, payload_size, type_id, color_id = struct.unpack(FRMT_AVATAR_HEADER, data)
    except struct.error:
        return None
    if tag != AVATAR_HEADER:
        return None
    if total_chunks > AVATAR_MAX_CHUNKS or payload_size > NETWORK_AVATAR_BYTES:
        return None
    return tag, player_id, avatar_id, total_chunks, payload_size, model_type_name(type_id), model_color_name(color_id)
...
def safe_unpack_gstart(data: bytes) -> Optional[Tuple[bytes, int, int, int, int, int]]:
    ...
    if legacy is not None:
        ...
        if tag != GSTART:
            return None
        now = int(time.time())
        return tag, 0, 0, DEFAULT_LEVEL_ID, 0, now
    ...
    if compat is not None:
        ...
        if tag != GSTART:
            return None
        now = int(time.time())
        return tag, countdown_id, match_id, DEFAULT_LEVEL_ID, 0, now
    if len(data) == GSTART_PACKET_SIZE_V2:
        unpacked = _safe_unpack_exact(data, FRMT_GSTART_V2)
        if unpacked is None:
            return None
        tag, countdown_id, match_id, selected_level, level_seed, start_sec = unpacked
        if tag != GSTART:
            return None
        return tag, countdown_id, match_id, normalize_level_id(selected_level), level_seed, int(start_sec) & UINT32_MAX
...
def pack_gend(
    ...
    """Each standing row: (player_id, placement, name, elapsed_centisec, platforms_reached)."""
    payload = struct.pack(FRMT_GEND_HEAD, GEND, int(match_id) & UINT32_MAX, reason_code, len(standings))
    for player_id, placement, name, elapsed_centisec, platforms in standings:
        payload += struct.pack(
            FRMT_GEND_ITEM,
            int(player_id),
            int(placement) & 0xFF,
            max(0, min(65535, int(platforms))),
            int(elapsed_centisec) & UINT32_MAX,
            _pack_name(name),
        )
    return payload


def safe_unpack_gend(data: bytes) -> Optional[Tuple[int, int, List[Tuple[int, int, str, int, int]]]]:
    """Returns standings as (player_id, placement, name, elapsed_centisec, platforms_reached)."""
    ...
    if len(data) >= legacy_head_size:
        try:
            ...
            tag, reason_code, count = None, 0, 0  # type: ignore[misc, assignment]
        else:
            if tag == GEND:
                expected = legacy_head_size + (count * GEND_ITEM_LEGACY_SIZE)
                if len(data) == expected:
                    standings_legacy: List[Tuple[int, int, str, int, int]] = []
                    offset = legacy_head_size
                    for _ in range(count):
                        player_id, placement, raw_name = struct.unpack(
                            FRMT_GEND_ITEM_LEGACY, data[offset : offset + GEND_ITEM_LEGACY_SIZE]
                        )
                        standings_legacy.append((player_id, placement, _unpack_name(raw_name), 0, 0))
                        offset += GEND_ITEM_LEGACY_SIZE
                    return 0, int(reason_code), standings_legacy
    ...
    body = len(data) - head_size
    if count == 0:
        if body != 0:
            return None
        return int(match_id) & UINT32_MAX, int(reason_code), []

    if body % count != 0:
        return None
    item_size = body // count

    rows: List[Tuple[int, int, str, int, int]] = []
    offset = head_size
    if item_size == GEND_ITEM_SIZE:
        for _ in range(count):
            player_id, placement, platforms, elapsed_cs, raw_name = struct.unpack(
                FRMT_GEND_ITEM, data[offset : offset + item_size]
            )
            rows.append(
                (player_id, placement, _unpack_name(raw_name), int(elapsed_cs) & UINT32_MAX, int(platforms))
            )
            offset += item_size
        return int(match_id) & UINT32_MAX, int(reason_code), rows

    if item_size == GEND_ITEM_LEGACY_SIZE:
        for _ in range(count):
            player_id, placement, raw_name = struct.unpack(FRMT_GEND_ITEM_LEGACY, data[offset : offset + item_size])
            rows.append((player_id, placement, _unpack_name(raw_name), 0, 0))
            offset += item_size
        return int(match_id) & UINT32_MAX, int(reason_code), rows
...
def pack_room_name_update(host_id: int, room_name: str) -> bytes:
    return struct.pack(FRMT_ROOM_NAME_UPDATE, ROOM_NAME_UPDATE, host_id, _pack_name(room_name))


def safe_unpack_room_name_update(data: bytes) -> Optional[Tuple[bytes, int, str]]:
    unpacked = _safe_unpack_exact(data, FRMT_ROOM_NAME_UPDATE)
    if unpacked is None:
        return None
    tag, host_id, raw_room_name = unpacked
