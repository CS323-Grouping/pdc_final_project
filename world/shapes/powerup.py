...
def create_orb_powerups_from_platform_specs(platforms, rng_seed: int = 42) -> list[PowerUp]:
    """Random-effect orbs above platforms; mirrors old level_1 density using current ``PlatformSpec`` layout."""

    import random

    rng = random.Random(rng_seed)
    seq = tuple(platforms)
    eligible = list(seq[3:]) if len(seq) > 3 else list(seq)
    if not eligible:
        eligible = list(seq)
    pick_n = max(1, len(eligible) // 4)
    pick_n = min(pick_n, len(eligible))
    selected = rng.sample(eligible, pick_n)
    out: list[PowerUp] = []
